...
//   expenses: Expenses[] = [];
//   displayed: Expenses[] = [];
//   selectedexpenses: Expenses[] | null = [];
...
//       const aValue = this.sortBy.key === 'title' ? a.title : this.sortBy.key === 'amount' ? a.amount : a.date;
//       const bValue = this.sortBy.key === 'title' ? b.title : this.sortBy.key === 'amount' ? b.amount : b.date; 
//       if (aValue < bValue) return this.sortBy.dir === 'asc' ? -1 : 1;
//       if (aValue > bValue) return this.sortBy.dir === 'asc' ? 1 : -1;
...
//       accept: () => {
//         const selectedIds = this.selectedexpenses?.map(p => p.id) || [];
//         this.expenses = this.expenses.filter(p => !selectedIds.includes(p.id));
